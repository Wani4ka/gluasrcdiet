...
local nil or repeat return then true until while continue]]):gmatch("%S+") do
  kw[v] = true
end
  ...
  while true do--outer
    ...
    while true do --luacheck: ignore 512
      ...
      if p then
        I = i + #r
		if r == "not" then
			addtoken("TK_OP", "not")				-- not bool unary operator
		elseif kw[r] then
