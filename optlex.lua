...
local function checkpair(i, j)
	...
	elseif (t1 == "TK_KEYWORD" and boolOps[sinfos[i]]) or
		 (t2 == "TK_KEYWORD" and boolOps[sinfos[j]]) then
	return ""
...
local function do_boolOp(i)
	if sinfos[i] == "and" then
		sinfos[i] = "&&"
	elseif sinfos[i] == "or" then
		sinfos[i] = "||"
	elseif sinfos[i] == "not" then
		sinfos[i] = "!"
	end
end
...
function M.optimize(option, toklist, semlist, toklnlist)
	...
	while true do
		...
		if semlist[i] == "and" or semlist[i] == "or" or semlist[i] == "not" then
				do_boolOp(i)
			end
			prev = i
	...
	-- Processing loop (PASS 3)
	if opt_x then
		i = 1
		-- Remove redundant commas before closing }s
		if stoks[1] == "TK_COMMENT" then
			-- first comment still existing must be shbang, skip whole line
			i = 3
		end
		while true do
			...
			elseif tok == "TK_OP" and info == "," then			 -- consider each }
				local t1, i1 = stoks[i + 1], sinfos[i + 1]
				if t1 == "TK_OP" and i1 == "}" then
					settoken()
				end
			end--if tok
